...
import { GroupBy } from './TimesheetFilters';
...
export function TimesheetGridView({ entries, dateRange, groupBy, onStartTimer, onRefresh }: TimesheetGridViewProps) {
  // Use weekly view: snap to week boundaries
    ...
    <div className="rounded-xl border border-border/50 bg-card/50 overflow-x-auto">
      <Table>
              ...
              <TableHead key={i} className="text-center min-w-[90px]">
                <div className="space-y-1">
                  ...
                  <div className="text-xs font-semibold text-primary">
                    {formatDuration(dayTotals[i])}
                  </div>
                  {/* Progress bar */}
                  <div className="h-1 w-full bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-primary rounded-full transition-all"
                      style={{ width: `${Math.min(100, (dayTotals[i] / (8 * 60)) * 100)}%` }}
                    />
                  </div>
                </div>
              </TableHead>
        ...
        <TableBody>
          {rows.length === 0 ? (
            <TableRow>
              <TableCell colSpan={days.length + 3} className="text-center text-muted-foreground py-12">
            ...
            return (
              <TableRow key={row.key} className="group">
                ...
                {days.map((day, i) => {
                  const dayEntries = row.entries.filter(e => isSameDay(parseISO(e.start_time), day));
                  const dayMin = dayEntries.reduce((s, e) => s + (e.duration_minutes || 0), 0);
                  return (
                    <GridCell
                      key={i}
                      minutes={dayMin}
...
function GridCell({
  ...
  const handleSave = async () => {
    ...
    if (entries.length > 0 && diff !== 0) {
      // Update the first entry's duration
