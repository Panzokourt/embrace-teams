...
import {
  CalendarDays, Play, Square, Plus, FolderKanban, FileText, Briefcase,
  GripVertical,
} from 'lucide-react';
...
export function MyWorkCalendar({
  ...
  // Separate tasks into all-day vs timed
  const allDayTasks = useMemo(() => tasks.filter(t => t.due_date && isAllDay(t.due_date)), [tasks]);
  const timedTasks = useMemo(() => tasks.filter(t => t.due_date && !isAllDay(t.due_date)), [tasks]);
  ...
  async function handleCreateTask() {
    ...
    if (createDialog.hour !== undefined) {
      dueDate.setHours(createDialog.hour, 0, 0, 0);
    }
    ...
    if (!projectId) {
      toast.error('Δεν βρέθηκε project');
      setCreating(false);
      return;
    }
      ...
      <Card className="border-border/30 shadow-sm">
        ...
        <CardContent className="p-0 overflow-auto max-h-[70vh]">
            ...
            {days.map(day => {
              ...
              return (
                <div
                  key={key}
                  className={cn(
                    'border-l border-border/20 px-1.5 py-2 text-center',
                    ...
                    <div className="mt-1 space-y-0.5">
                      {dayMilestones.slice(0, 3).map(m => (
                        <div
                          key={m.id}
                          className={cn(
                            'text-[9px] leading-tight px-1.5 py-0.5 rounded-full border truncate',
                            MILESTONE_COLORS[m.type]
                          )}
                          title={m.label}
                        >
                          {m.label}
                      ...
                      {dayMilestones.length > 3 && (
                        <div className="text-[9px] text-muted-foreground">+{dayMilestones.length - 3}</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className={cn(
            ...
            <div className="px-2 py-1.5 text-[10px] text-muted-foreground text-right pr-3">
              Ολοήμ.
            </div>
            {days.map(day => {
              const key = format(day, 'yyyy-MM-dd');
              const dayAllDay = allDayTasks.filter(t => t.due_date && isSameDay(new Date(t.due_date), day));
              return (
                <div
                  key={`allday-${key}`}
                  className="border-l border-border/20 px-1 py-1 min-h-[36px] hover:bg-accent/10 transition-colors cursor-pointer"
          ...
          {HOURS.map(hour => (
            <div
              key={hour}
              className={cn(
                'grid border-b border-border/10',
                calendarMode === 'week' ? 'grid-cols-[60px_repeat(7,1fr)]' : 'grid-cols-[60px_1fr]',
                'min-h-[52px]'
              )}
            >
              <div className="px-2 py-1 text-[10px] text-muted-foreground text-right pr-3 -mt-1.5 tabular-nums">
              ...
              {days.map(day => {
                ...
                return (
                  <div
                    key={`${key}-${hour}`}
                    className={cn(
                      'border-l border-border/20 px-0.5 py-0.5 cursor-pointer hover:bg-accent/10 transition-colors',
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={e => { e.preventDefault(); handleCreateTask(); }} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={!newTaskTitle.trim() || creating} className="gap-1.5">
...
function CalendarTaskPill({
  task, onDragStart, onClick, activeTimer, startTimer, stopTimer, showTime,
}: {
  task: TaskItem;
  ...
  const isOverdue = task.due_date && isBefore(startOfDay(new Date(task.due_date)), startOfDay(new Date())) && task.status !== 'completed';
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        'group flex items-center gap-1 rounded-[6px] border px-1.5 py-1 mb-0.5 cursor-grab active:cursor-grabbing transition-colors text-[11px]',
        isOverdue
          ? 'border-destructive/30 bg-destructive/5 text-destructive'
          : 'border-border/30 bg-card hover:bg-accent/20'
      )}
    >
      ...
      {!isRunning ? (
        <button
          className="h-4 w-4 flex items-center justify-center text-muted-foreground hover:text-primary shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
          onClick={e => { e.stopPropagation(); startTimer(task.id, task.project_id); }}
        >
          <Play className="h-2.5 w-2.5" />
        </button>
      ) : (
        <button
          className="h-4 w-4 flex items-center justify-center text-primary shrink-0"
